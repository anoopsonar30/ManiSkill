...
# Load sysid config at module level for use in controller configs
def _load_sysid_config():
    if SYSTEM_IDENTIFICATION_JSON_PATH.exists():
        with open(SYSTEM_IDENTIFICATION_JSON_PATH, "r") as f:
            return json.load(f)
    return None

_SYSID_CONFIG = _load_sysid_config()
...
class Panda(BaseAgent):
    ...
    urdf_path = f"{PACKAGE_ASSET_DIR}/robots/fr3/fr3_agilex_gripper.urdf"
    urdf_config = dict(
        _materials=dict(
            gripper=dict(static_friction=2.0, dynamic_friction=2.0, restitution=0.0)
        ),
        link=dict(
            fr3_left_finger=dict(
                material="gripper", patch_radius=0.1, min_patch_radius=0.1
            ),
            fr3_right_finger=dict(
                material="gripper", patch_radius=0.1, min_patch_radius=0.1
            ),
        ),
    )
    ...
    ee_link_name = "fr3_gripper_tcp"

    # Use sysid values for arm gains/friction if available, otherwise fall back to defaults
    arm_stiffness = _SYSID_CONFIG["p_gain"] if _SYSID_CONFIG else 1e3
    arm_damping = _SYSID_CONFIG["d_gain"] if _SYSID_CONFIG else 1e2
    arm_friction = _SYSID_CONFIG["friction"] if _SYSID_CONFIG else 0.0
    arm_force_limit = 100
    ...
    def _after_loading_articulation(self):
        """Apply system identification armature after loading the robot.
    ...
    def _after_init(self):
        self.finger1_link = sapien_utils.get_obj_by_name(
            self.robot.get_links(), "fr3_left_finger"
        )
        self.finger2_link = sapien_utils.get_obj_by_name(
            self.robot.get_links(), "fr3_right_finger"
        )
